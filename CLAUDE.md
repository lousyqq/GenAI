...
- **人人可瀏覽與自動開戶與人員資訊連動**：`Auth:AutoProvisionWindowsAccounts=true` — 當 Windows 偵測或模擬的工號不在 Accounts 表時，自動建立 user 帳號並指派 `Auth:DefaultRoleIds`（目前 `role_1`）；同時自動至 `[WEB].[dbo].[notes_person]` 比對 `EMPNO = 登入帳號`（或純數字工號模糊查詢），將 `DEPTNAME` 寫入部門欄位、`NAME` 寫入姓名欄位（若 DB 查無此工號資料，姓名維持登入帳號、部門維持空白）。此外在新增/編輯帳號與舊有帳號載入（`FindAccountAsync`）時亦同步適用此自動解析與回填規則，並提供 `GET /api/Accounts/LookupPerson` 供前端「新增帳號」表單即時帶入。

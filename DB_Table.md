...
IF OBJECT_ID('dbo.SiteVisitorDailyStats') IS NULL
CREATE TABLE dbo.SiteVisitorDailyStats (
    StatDate       date          NOT NULL,
    EmpId          nvarchar(50)  NOT NULL,
    EmpName        nvarchar(100) NULL,
    Department     nvarchar(100) NULL,
    PageViews      int           NOT NULL CONSTRAINT DF_SiteVisitorDailyStats_PageViews DEFAULT (1),
...
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name='IX_SiteVisitorDailyStats_EmpId')        CREATE INDEX IX_SiteVisitorDailyStats_EmpId        ON dbo.SiteVisitorDailyStats (EmpId);
...
-- ==================== 5. 架構變更歷程（增量 ALTER） ====================
-- 讓已執行過之前完整建置 SQL 的遠端 DB 可直接跑此處增量升級指令（冪等設計）。

-- [2026-07-18] 新增：網站每日/月份瀏覽彙總統計表與對應索引
IF OBJECT_ID('dbo.SiteVisitorDailyStats') IS NULL
BEGIN
    CREATE TABLE dbo.SiteVisitorDailyStats (
        StatDate       date          NOT NULL,
...
IF OBJECT_ID('dbo.SiteVisitorDailyStats') IS NOT NULL
   AND NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_SiteVisitorDailyStats_EmpId' AND object_id = OBJECT_ID('dbo.SiteVisitorDailyStats'))
BEGIN
...
### SiteVisitorDailyStats — 網站每日/月份瀏覽彙總統計
| 欄位 | 型別 | Null | Key |
|---|---|---|---|
| StatDate | date | NO | PK |
| EmpId | nvarchar(50) | NO | PK |
| EmpName | nvarchar(100) | YES | |
| Department | nvarchar(100) | YES | |
| PageViews | int | NO | Default(1) |
| FirstVisitTime | datetime2 | NO | |
| LastVisitTime | datetime2 | NO | |
